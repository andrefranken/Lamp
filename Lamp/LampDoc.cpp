...
void CLampDoc::ReadProfile(const char *pText, int datasize)
{
   ...
   if(i != n.end())
   {
      ...
      if(data.Length() > 4)
         data.TrimEnd(data.Length() - 4);
      text += data;
               
      text += L" (";
      FindValue(data, i, L"posterclass");
      text += data;
      text += L")\n";
      ...
      text += L"l{ShackWordCloudl{";
      FindValue(data, i, L"url_shackwordcloud");
      text += data;
      text += L"}l}l\n";

      text += L"l{ChattyPics Galleryl{";
      FindValue(data, i, L"url_chattypicsgallery");
      text += data;
      text += L"}l}l\n";
